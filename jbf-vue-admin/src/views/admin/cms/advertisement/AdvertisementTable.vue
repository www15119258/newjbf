<template>
  <div>
    <el-row :gutter="20">
      <el-col :span="24" class="toolbar">
        <el-form :inline="true" :model="queryForm">
          <el-form-item v-if="showQueryBtn && $hasPerm('jbf.cms.advertisement.view')"  label="名称：">
            <el-input v-model="queryForm.name" placeholder="名称" style="width:140px;"></el-input>
          </el-form-item>
          <el-form-item v-if="showQueryBtn && $hasPerm('jbf.cms.advertisement.view')"  label="类型：">
            <dict-select v-model="queryForm.type" initType="cms_advertisement_type"></dict-select>
          </el-form-item>
          <el-form-item v-if="showQueryBtn  && $hasPerm('jbf.cms.advertisement.view')"  label="状态：">
          ...
          <el-form-item>
            <el-button-group>
              <el-button v-if="showQueryBtn && $hasPerm('jbf.cms.advertisement.view')" type="primary" @click="handleQuery">查询</el-button>
              <el-button v-if="showAddBtn && $hasPerm('jbf.cms.advertisement.add')" type="primary" @click="handleAdd">新增</el-button>
              <el-button v-if="showEditBtn && $hasPerm('jbf.cms.advertisement.edit')" type="primary" @click="handleEdit">编辑</el-button>
              <el-button v-if="showDeleteBtn && $hasPerm('jbf.cms.advertisement.delete')" type="primary" @click="handleDelete">删除</el-button>
            </el-button-group>
          </el-form-item>
        </el-form>
      </el-col>
    ...
    <el-row :gutter="20">
      <el-col :span="24">
        <el-table v-if="showQueryBtn && $hasPerm('jbf.cms.advertisement.view')" ref="dataTable" :data="dataTable" :highlight-current-row="true" @current-change="selectChange" v-loading="loading"
    ...
    <el-dialog v-el-drag-dialog :close-on-click-modal="false" :title="dialogTitle" :visible.sync="dialogVisible" @close="closeDialog">
      <el-form :model="form" ref="form" :rules="rules">
        ...
        <el-form-item label="类型：" labelWidth="120px" prop="type">
          <dict-select v-model="form.type" initType="cms_advertisement_type" style="width: 100%;" :clearable="true"></dict-select>
        </el-form-item>
...
<script>
import { queryAdvertisement, addAdvertisement, updateAdvertisement, deleteAdvertisement } from '../api/advertisement.js'
...
export default {
  ...
  methods: {
    handleQuery: function () {
      ...
      queryAdvertisement(this.queryForm).then((response) => {
    ...
    handleDelete: function () {
      ...
      this.$confirm('您确定要删除该记录吗？', '提示', {
        ...
        deleteAdvertisement(this.currentRow.id).then((response) => {
