<template>
  <div>
    ...
    <el-dialog v-el-drag-dialog :close-on-click-modal="false" title="请选择" :visible.sync="dialogVisible" append-to-body>
      <post-table ref="postTable" :dblclick="dblclick" :showQueryBtn="true" ></post-table>
...
import { queryPostById } from '../api/post.js'
...
export default {
  ...
  methods: {
    ...
    submitChoose: function () {
      let row = this.$refs['postTable'].getCurrentRow()
    ...
    setId: function (val) {
      if (val === undefined || val === null || val === '') {
        ...
        queryPostById(val).then((response) => {
