  ...
  <div v-loading.fullscreen.lock="loading">
    <el-row :gutter='20'>
      <el-col :span='24' class='breadcrumb-container'>
        <strong class='title'>编辑页面</strong>
...
import { updateSpage, querySpageById } from '../api/spage.js'
...
export default {
  ...
  methods: {
    ...
    goSubmit: function () {
      ...
      this.$refs['form'].validate((valid) => {
        if (valid) {
          this.loading = true
          updateSpage(this.form).then((response) => {
            this.loading = false
  ...
  mounted: function () {
    let spageid = this.$route.params.id
    querySpageById(spageid).then((response) => {
      if (response && response.data && response.data.status === 'success') {
        ...
        this.form.template = response.data.data.template
