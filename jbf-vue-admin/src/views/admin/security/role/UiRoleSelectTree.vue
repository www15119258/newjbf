<template>
  <div>
    ...
    <el-dialog v-el-drag-dialog :closeOnClickModal="false" title="选择" :visible.sync="dialogVisible" appendToBody>
      <dict-select v-show="changeTypeEnable" v-model="type" initType="sys_role_type" style="width: 100%; margin-bottom: 5px" :clearable="false" @change="roleTypeChange"></dict-select>
...
<script>
import { queryRoleTree, queryRoleById } from '../api/role'
...
export default {
  ...
  methods: {
    setVal: function (val) {
      ...
      queryRoleById(val).then((response) => {
        if (response && response.data && response.data.status === 'success') {
          let role = response.data.data
          this.selectVal = role.nickname
        }
      })
    },
    ...
    dialogSubmit: function () {
      ...
      let oldVal = this.val
      this.selectVal = this.currentData.nickname
